...
sub update {
    ...
    for(@ok_fields) {
        my $attr = $obj->meta->get_attribute($_);
        push @values, ( $attr->serializer ? $attr->serializer->($obj->$_) : $obj->$_ );
    }
    
    return $self->_update($table, $key_field, $key_value, \@ok_fields, \@values);
}
=head2 delete($obj)
